...
fn main() {
    ...
    let mut temple = Temple::new(spot.gfx.video.profile);
    ...
    'gameloop: loop {
        ...
        for event in spot.events.poll_iter() {
            match event {
                ...
                sdl2::event::Event::MouseMotion { xrel, yrel, .. } => {
                    let node = temple.model.nodes.get_mut(&temple.camera).unwrap();
        ...
        spot.gfx
            .renderer
            .draw(&temple.model, &temple.root, &na::Matrix4::identity());

        let frame = spot.gfx.next_frame();
        spot.gfx
            .renderer
            .render_shadow(&temple.model, &frame.shadow_buffer);
        ...
        spot.gfx
            .renderer
            .render_geometry(&temple.model, &frame.geometry_buffer);
